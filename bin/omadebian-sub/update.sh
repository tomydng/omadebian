...
CHOICES=(
	"Omadebian        Update Omadebian itself and run any migrations"
...
if [[ "$CHOICE" == "<< Back"* ]] || [[ -z "$CHOICE" ]]; then
	...
	case "$INSTALLER" in
	"omadebian") INSTALLER_FILE="$OMADEBIAN_PATH/bin/omadebian-sub/migrate.sh" ;;
	"ollama") INSTALLER_FILE="$OMADEBIAN_PATH/install/terminal/optional/app-ollama.sh" ;;
	*) INSTALLER_FILE="$OMADEBIAN_PATH/install/terminal/app-$INSTALLER.sh" ;;
	esac
